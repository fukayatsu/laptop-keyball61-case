...
固定式v2とは別系統の、**中空モジュール5分割＋ネジ結合**の構成。`scad/v3/`。
...
| パーツ | サイズ/体積 | 印刷 | 説明 |
|---|---|---|---|
| `stl/v3_kb_box_left/right.stl` | 155×126×25 / 63cm³ | 正立 | **KBフレーム**: 天板なし＝マウントプレートが天板になるモノコック。周壁+座リング+米の字スポーク(水平/垂直/対角2, ボスはスポーク上)+貫通ボス4+BCネジ筒柱2 |
| `stl/v3_palm_box_left/right.stl` | 108×88×20 / 36cm³ | **伏せ**(回転済) | パームシェル。差し替え式(形を変えたらこれだけ再印刷) |
| `stl/v3_center_box.stl` | 143×162×16 / 74cm³ | **伏せ**(回転済) | 台形トーションボックス(ブリッジ+ブレース代替)。ポッド直載せ・ベルクロ/結束バンド(3×14×2穴×4組)スロット・後付け底蓋ボス付き |
| `stl/v3_center_lid.stl` | 136×156×2.5 / 42cm³ | 平置き | 後付け底蓋(任意)。M3×8皿×4で面一固定、閉断面化 |

- 全パーツ**サポート・ブリッジ実質不要**
- 内部ウェブは `v3_web_style` で米の字(既定)/井の字を切替可
- マウントプレート・ポッドはv2と共用。ネジは全箇所M3×8の**垂直締結**——
  ジョイントは「デッキ/天板と面一のタブ＋KBフレーム側の切り欠き＋横挿しナットスロット」
  方式で、**すべて上から普通のドライバーで締められる**（六角レンチ不要）
- ジョイント12本（パーム3/側＋中央3/側）は**M3×8なべ小ねじ**: 厚3mmのタブに
  皿もみを入れると縁が刃状に薄くなるため、平座面のなべ頭で締める（頭の出っ張りは
  切り欠き内に収まり、パーム側はジェルパッドの下）。その他（プレート・ポッド・蓋）は
  v2と共通のM3×8皿

**組立順**（すべて上からのネジ作業。下からはキーボードの純正ネジのみ）:
1. KBフレームのナットスロット6箇所（東壁3=チャネル側から・南壁3=内側から）に
   M3ナットを横挿し
2. パームのタブをKB南壁の切り欠きに載せ、上からM3×8なべ×3（頭はジェルパッドの下）
3. 中央箱のタブをKB東壁の切り欠きに載せ、上からM3×8なべ×3/側
4. マウントプレート→フレーム: 上からM3×4/側
5. キーボード→プレート: 下から純正M2×7 → ボールケースを上から挿入し下からBCネジ×2
6. ポッド: トレイ→中央天板M3×2 → モジュール/ケーブル → 蓋M3×2。底蓋は任意でM3×4
