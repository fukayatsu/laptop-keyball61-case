...
- **パームレスト間は開放**: 前方中央は material レスの V 字空間。全体 約447×223mm
...
- **ケースはアクリル底板（Bottom Acryl）の置き換え**: 取付面の彫り込みは実寸 DXF から起こした
  ...
  **純正の M2 ネジとボールケースネジをそのまま使って**下から直接固定できる。
  ボール PCB の切り欠き部は取付面からさらに 4mm 掘り下げて逃げ済み

## パーツ構成（印刷物・すべて PLA / サポート不要）

| ファイル | サイズ (mm) | 数 | 備考 |
|---|---|---|---|
| `stl/side_left.stl` | 172×205×24 | 1 | 取付面+パームレスト+7°楔+連続継手タブ(凸)。印刷向きに回転済み |
| `stl/side_right.stl` | 172×205×24 | 1 | 左のミラー |
| `stl/bridge.stl` | 142×162×8 | 1 | 台形。下面に連続継手ポケット(凹・リブ入り)・ポッド取付穴付き |
| `stl/brace.stl` | 155×62×3.5 | 1 | 左右サイドを上側で連結する台形プレート(閉断面化)。TRSジャック部(y>162)は回避 |
...
| 用途 | 品名 | 数量 | 備考 |
|---|---|---|---|
| 共通ネジ | **M3×8 皿小ねじ** | **14 (+予備)** | 継手 6（下面から・ナット締結）/ ブレース 4・ポッド固定 2・蓋 2（セルフタップ） |
...
底板アクリル DXF（3:1 スケール）から実寸化。左右ともボール搭載なので BallSide 形状を両側に使用（右はミラー）。

## 印刷設定の目安

- PLA / レイヤー 0.2〜0.28mm / 壁 3 周 / インフィル 12〜15%
- サイドは体重がかかるためインフィルより壁数を増やす方が効く
...
4. モジュールを入れ、TRS ケーブルと Touch ID ボタンの延長ケーブルをギャラリーに
   通してから蓋を M3×8 で閉じる
5. ブレースプレートをサイドの座面に載せ、上から M3×8 皿 ×4 で固定（セルフタップ）
