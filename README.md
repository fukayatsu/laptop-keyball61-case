...
- たわみ抑えリブ: ジェルパッド80×95mm領域(パーム外側の隅)の北縁+外側エッジの
  L字(パッドの位置決めストッパー兼用) + **スパイン接続帯の横リブ×2**(蝶番になる
  帯を曲げに直交する向きで補強)
- ケーブル関連: 外側後方の角に**ベルトループ**(上面のバー下にバンドを通す。
  貫通穴だとバンドが足に当たるため)、親指M2穴の真北に3×15貫通スロット、
  TRRSジャック真東にTRSケーブル通し穴10×33(プラグごと裏へ通しカバーのTRSノッチへ)、
  カバー北縁にUSBノッチ
