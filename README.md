...
| パーツ | 印刷 | 説明 |
|---|---|---|
| `stl/v4_plate_left/right.stl` | ベタ置き | **一体平面プレート**(2mm): キーボード搭載部+パームレスト+中央張り出し。完全な平板(フェンスなし) |
...
- **下面カバー**: スパイン底面が蓋になり、下の空間(深さ約10mm)にケーブルや
  Touch IDモジュール等を収納。下からM3×8なべ×4(ボス内深座ぐり→スパイン底面の
  下穴へセルフタップ、掛かり約5mm)。北縁にUSB・東西縁にTRSのケーブルノッチ。
  スパインの結束バンドスロット対×4は内容物の固定に使える
- 外側後方の角(左サイドの左上/右サイドの右上)に**ケーブルまとめ用の3×15スロット**
- スプレイ10°・左右間隔はv3と同じ短縮値(`v4_corner_x=20`)
- 注意: 平面構造なのでプレートは接地ライン(外側エッジ)以外浮く。膝上での使用前提
- ネジ: M3×8なべ×6 + M3ナット×6 (+カバー用M3×8×4) + 純正キーボードネジ
